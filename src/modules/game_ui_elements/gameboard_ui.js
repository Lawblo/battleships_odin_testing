...
const create_gamebpard_coords = () => {
  ...
  for (let i = 1; i < 8; i++) {
    ...
    coord_container
      .classList.add('coord-container')
    coord_container.textContent = num;
    coords
      .appendChild(coord_container)
  }
  return coords
}
...
const create_gameboard_inner = () => {
  ...
  for (let y = 6; y > -1; y--) {
    for (let x = 0; x < 7; x++) {
      const board_square = create_square(x, y)
      gameboard_inner.appendChild(board_square)
    }
  }
...
const create_square = (x, y) => {
  ...
  get_square_location(x, y)
    .forEach(location => square.classList.add(location))
...
const get_square_location = (x, y) => {
  let location = []
  if (x == 0) { location.push('square-left') }
  else if (x == 6) { location.push('square-right') }

  if (y === 0) { location.push('square-bottom') }
  else if (y === 6) { location.push('square-top') }
